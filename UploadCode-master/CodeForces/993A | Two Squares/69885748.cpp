...
int main(int argc, const char * argv[])
{
    ...
    for(int i=0;i<4;++i)
    {
        scanf("%lf %lf",&drept[i].x,&drept[i].y);
        drept[i].x+=100;
        drept[i].y+=100;
    }
    for(int i=0;i<4;++i)
    {
        scanf("%lf %lf",&romb[i].x,&romb[i].y);
        romb[i].x+=100;
        romb[i].y+=100;
    }
    sort(drept,drept+4,mycmp);
    sort(romb,romb+4,mycmp);
    D.dr=drept[3];
    D.dr2=drept[2];
    D.st2=drept[1];
    D.st=drept[0];
    ...
    bool ok=0;
    for(int i=0;i<4;++i)
        if(punct_interior_romb(R,drept[i]))
            ok=1;
    for(int i=0;i<4;++i)
        if(punct_interior_dreptunghi(D,romb[i]))
            ok=1;
    if(punct_interior_dreptunghi(D, R.mij))
        ok=1;
    if(ok==0)
        printf("NO");
    else
