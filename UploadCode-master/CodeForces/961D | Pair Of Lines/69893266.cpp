...
POINT v[100005];
bool verif(int ind1,int ind2,int n)
{
    vector<POINT> v2;
    for(int i=1;i<=n;++i)
        if(i==ind1 || i==ind2 || aria_triunghi_directionat(v[ind1], v[ind2], v[i])<eps)
            continue;
        else
            v2.push_back(v[i]);
    if(v2.size()<=2)
    ...
    for(int i=2;i<v2.size();++i)
        if(aria_triunghi_directionat(v2[0], v2[1], v2[i])>=eps)
            return 0;
    return 1;
}
int main(int argc, const char * argv[])
{
    ...
    if(verif(1,3,n) || verif(1,2,n) || verif(2,3,n))
        printf("YES");
    else
        printf("NO");
    return 0;
