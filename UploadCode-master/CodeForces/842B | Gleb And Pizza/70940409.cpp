...
int main(int argc, const char * argv[])
{
    ...
    int n,i,nr=0;
    CERC interior,exterior;
    interior.C.x=interior.C.y=exterior.C.x=exterior.C.y=0.0;
    scanf("%lf%lf",&exterior.r,&interior.r);
    interior.r=exterior.r-interior.r;
    scanf("%d",&n);
    for(i=1;i<=n;++i)
    {
        CERC c;
        scanf("%lf%lf%lf",&c.C.x,&c.C.y,&c.r);
        double d=dist(c.C,interior.C);
        if(d-c.r>=interior.r && d+c.r<=exterior.r)
            nr++;
    }
    printf("%d",nr);
    return 0;
