...
int main(int argc, const char * argv[])
{
    POINT fin;
    CERC C;
    scanf("%lf%lf%lf%lf%lf",&C.r,&C.C.x,&C.C.y,&fin.x,&fin.y);
    if(round(dist(C.C, fin)/(2*C.r))-dist(C.C, fin)/(2*C.r)>eps || fabs(round(dist(C.C, fin)/(2*C.r))-dist(C.C, fin)/(2*C.r))<eps)
        printf("%.0lf",round(dist(C.C, fin)/(2*C.r)));
    else
        printf("%.0lf",round(dist(C.C, fin)/(2*C.r))+1.0);
    return 0;
}