...
class TNode:
    ...
    ## This method does not work, fix it
...
class DTree:
    ...
    def add(self, data):
        ...
        """
        # 1. empty tree? special case
        if self.root is None:
            self.root = TNode(data)
            return self.root
        # 2. find a suitable parent by breadth-first search
        parent = self._get_available_node(self.max_degree)
        # 3. create a new node with the found parent and return it
        return TNode(data, parent)

    def _get_available_node(self, max_degree):
        # Search for the first node with fewer than max_degree children and return it.
        # The search is breadth-first: it goes layer-by-layer
        assert self.root is not None
        # create a set of candidates with just the root
        candidates = {self.root}
        while True:
            # 1. check if one of the candidates is available,
            #    and if so return it
            for c in sorted(candidates):
                if len(c.children) < max_degree:
                    return c
            # 2. put all the candidates' children in a new set
            newcandidates = set()
            for c in sorted(candidates):
                newcandidates.update(c.children)
            # 3. that set is now the new candidate set
    ...
    def attach_tree(self, other):
        ...
        if self.root is None:
            ...
            parent = self._get_available_node(self.max_degree)
            other.root.parent = parent
            parent.children.add(other.root)
        other.root = None
        return None
